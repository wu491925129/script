...
function getFlag(res) {
    ...
    res.data[0].time[0].times.forEach(function (item) {
        if (!item.fullFlag) {
            pushContent = encodeURI(item.select_msg)
            hasFlag = true;
        }
    })
    return hasFlag;
